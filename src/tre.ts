      ...
      if (parsed.type === "text_done" && parsed.content) {
        ...
        ttsWs.on("open", async () => {
          ...
          for (let i = 0; i < chunks.length; i++) {
            const flushNow = i === chunks.length - 1;
            ...
            // Kirim teks sementara ke client saat audio lagi jalan
            clientSocket.send(
              JSON.stringify({
                type: "tts_text",
                text: chunks[i],
...
console.log(`[${sessionId}] [TTS] Sent text chunk ${turnId} ${i + 1}/${chunks.length}:`, chunks[i]);
            await delay(100); // biar audio stream gak numpuk
          }
        });
...
if (data.audio && ttsWs === currentTTS) {
  ...
  console.log(
    `[${sessionId}] [TTS] Sent audio chunk ${turnId}: ${data.audio.length} bytes, ~${durationMs} ms`
  );
}


    // ✅ Kirim alignment data (sekali saja)
    if ((data.alignment || data.normalizedAlignment) && ttsWs === currentTTS) {
      ...
      console.log(`[${sessionId}] [TTS] Sent alignment ${turnId} :`, alignment);
    }
